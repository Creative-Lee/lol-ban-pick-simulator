...
export const getAscendingChampionDataList = async (recentVersion) => {
  try {
    const response = await lolApi.get(
      `cdn/${recentVersion}/data/ko_KR/champion.json`
    )
...
export const getClassicSpell = async (recentVersion) => {
  try {
    const response = await lolApi.get(
      `cdn/${recentVersion}/data/ko_KR/summoner.json`
    )
    const spellData = response.data.data
    const spellsPropertyList = Object.values(spellData)
    const classicSpellList = spellsPropertyList.filter((property) =>
      property.modes.includes('CLASSIC')
    )
...
export const getDownloadResultPngFile = async (elementId) => {
  try {
    ...
    const capturedCanvas = await html2canvas(
      document.getElementById(elementId),
      {
        useCORS: true,
        allowTaint: true,
        onclone: (document) =>
          captureErroredInputId.forEach(
            (inputId) =>
              (document.getElementById(inputId).style.fontSize = '20px')
          ),
      }
    )
